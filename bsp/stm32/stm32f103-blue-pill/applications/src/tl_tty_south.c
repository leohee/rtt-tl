...
static void tl_tty_south_recv_entry (void *data)
{
...
rt_err_t tl_tty_south_thread (struct tl_tty_t *pTTY)
{
	rt_err_t ret = RT_EOK;
	rt_thread_t	tid_tty;
	...
	rt_mq_init(&pTTY->rx_mq, "south_rx_mq", pTTY->msg_pool, 
	...
	rt_device_set_rx_indicate(pTTY->dev, tl_tty_south_input);
